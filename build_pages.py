...
<footer class="site">
  <div class="wrap foot-grid">
    ...
    <div>
      ...
      <a href="/policies.html#accessibility">Accessibility</a>
    </div>
  </div>
...
def render_product_page(p, catalog):
    ...
    # The thumbnail's accessible name comes from the button, not the <img>, so the
    # image stays alt="" and the label says which photo of which product it shows.
    n_photos = len(p["images"])
    thumbs = "\n".join(
        '<button class="thumb thumb-photo" aria-label="Show photo %d of %d, %s">'
        '<img loading="lazy" src="%s" alt=""></button>'
        % (i + 1, n_photos, esc(p["title"]),
           esc(src) if src.startswith("http") else "/" + esc(src))
...
<main class="wrap pdp" id="main" tabindex="-1">
  <div class="gallery">
